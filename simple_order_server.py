#!/usr/bin/env python3
"""Simple order server aligned with current protocol.

Protocol Notes:
  - Market data messages (quotes/trades) have a 2-byte header (len,type).
  - OUTGOING order messages (from trader to server) have NO header and are a
    fixed 25 bytes (see PROTOCOL_SPEC.md). Previous version incorrectly
    expected a 5-byte header. This server now reads raw 25-byte messages.
"""
...
def main():
    ...
    print(f"Order server listening on 127.0.0.1:{port} (expecting raw {ORDER_SIZE}-byte orders)")
